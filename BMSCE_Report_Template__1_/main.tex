...
\lstdefinestyle{cqlstyle}{
  ...
  morekeywords={CREATE,KEYSPACE,IF,NOT,EXISTS,WITH,REPLICATION,AND,TABLE,
                PRIMARY,KEY,CLUSTERING,ORDER,BY,ASC,DESC,INSERT,INTO,VALUES,
                SELECT,FROM,WHERE,LIMIT,DELETE,UPDATE,SET,CLASS,
                SimpleStrategy,text,int,double,ALLOW,FILTERING},
}
\lstdefinestyle{bashstyle}{
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  breaklines=true, frame=single,
  rulecolor=\color{codegray}, xleftmargin=8pt,
}
...
\usepackage{fancyhdr}
\usepackage{setspace}
\usepackage{eso-pic}         % reliable shipout-level background drawing
% ------ Professional page border (consistent on every page) -----------------
...
\usepackage[colorlinks=true,linkcolor=blue!50!black,
            urlcolor=blue,citecolor=green!40!black,
...
% ------ Page style -----------------------------------------------------------
\setlength{\headheight}{15pt}        % silence fancyhdr warning
\pagestyle{fancy}
\fancyhf{}
\rhead{\small\textit{NIDS --- DevOps Lab Report}}
\lhead{\small Ullas N \textbar{} H S Bhat \textbar{} R V Abhishek}
\cfoot{\thepage}
...
\includegraphics[scale=0.6]{logo.png}\\[0.4cm]
...
year 2024--2025, in partial fulfilment of the requirements for the award of
...
\vfill
\noindent\textbf{Date:}\hspace{1cm}April 2025
...
\begin{enumerate}
  ...
  \item Receive instant live notifications the moment a high-severity threat
        (DoS, Exploits, Shellcode, Backdoors) is detected via Redis pub/sub.
\end{enumerate}

The system uses the \textbf{UNSW-NB15 dataset} from the University of New South
Wales Canberra---a benchmark network security dataset with 2.5 million real
traffic records, 42 feature columns, and 9 distinct attack categories.
...
\subsection{Cassandra \texttt{nids.alerts} Table}
...
\subsection{Cassandra \texttt{nids.sessions} Table}
\begin{table}[H]
  \centering
...
\begin{itemize}
  ...
  \item Feature set: \textbf{39 numeric} $+$ \textbf{3 categorical} (proto,
...
\begin{itemize}
  \item \textit{High:}   DoS, Exploits, Shellcode, Backdoors
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{Dashboard.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{Alerts.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{Settings.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|p{6cm}|}
    ...
    Docker Compose        & v2.24.6                & Multi-container application
      deployment \\
    \hline
    ngrok                 & 3.36.1                 & Static HTTPS tunnel exposing
    ...
    \hline
    Python                & 3.13.5 (in Jenkins)    & Runtime for linting and
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 143030}.png}
  \caption{Jenkins Blue Ocean pipeline view for Build~\#7 --- all ten stages green (Checkout SCM \textrightarrow{} Install Dependencies \textrightarrow{} Quality [Lint + Unit Tests in parallel] \textrightarrow{} Integration Tests \textrightarrow{} Clean Docker \textrightarrow{} Verify Workspace \textrightarrow{} Build Docker Image \textrightarrow{} Deploy \textrightarrow{} Post Actions), overall result: SUCCESS in 5\,min\,8\,sec}
  \label{fig:stages}
\end{figure}
...
\begin{enumerate}
  ...
  \item GitHub fires a webhook \texttt{POST} to the static ngrok domain:\\
        \small\url{https://unattackable-agueda-unauthoritatively.ngrok-free.app/github-webhook/}
...
\subsection{Verified Build Result: Build \#8}

\begin{table}[H]
  \centering
  \caption{Jenkins Build \#8 stage-by-stage results (fully green)}
  \label{tab:build8}
  \begin{tabular}{|l|c|l|}
    \hline
    \textbf{Stage}           & \textbf{Status} & \textbf{Detail} \\
    \hline
    Checkout                 & \textcolor{codegreen}{\textbf{PASSED}} &
      Latest master commit pulled \\
    \hline
    Install Dependencies     & \textcolor{codegreen}{\textbf{PASSED}} &
      All packages from requirements.txt installed \\
    \hline
    Lint                     & \textcolor{codegreen}{\textbf{PASSED}} &
      \textbf{0 flake8 errors} \\
    \hline
    Test                     & \textcolor{codegreen}{\textbf{PASSED}} &
      \textbf{38 / 38 tests passed}, 0 failures \\
    \hline
    Build Docker Image       & \textcolor{codegreen}{\textbf{PASSED}} &
      Image \texttt{nids-app:8} and \texttt{nids-app:latest} created \\
    \hline
    Deploy                   & \textcolor{codegreen}{\textbf{PASSED}} &
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 142917}.png}
  \caption{Jenkins\texttt{ nids-pipeline} status page --- Build~\#7 successful (green badge), build artefacts (\texttt{coverage.xml}, \texttt{integration-results.xml}, \texttt{test-results.xml}), and Latest Test Result showing \textbf{no failures}. Right panel shows Test Result Trend and Code Coverage Trend graphs across builds \#1--\#7.}
  \label{fig:build-success}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 142936}.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|l|p{4.2cm}|}
    ...
    \hline
    \texttt{kafka}     & \texttt{cp-kafka:7.5.0}             & 19092 (ext),
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 143141}.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 143141}.png}
  \caption{Jenkins console log for Build~\#7 Post\,Actions stage --- confirms BUILD SUCCEEDED, followed by automated cleanup (\texttt{rm -rf venv} and \texttt{docker image prune}).}
  \label{fig:console}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 142759}.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{{Screenshot 2026-04-23 143049}.png}
...
\begin{enumerate}
  \item Murtaza Baig, Jill Slay, et al.\ (2015).
